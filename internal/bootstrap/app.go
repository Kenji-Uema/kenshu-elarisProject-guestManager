...
func BuildApp(ctx context.Context, cfg config.Configs) (*App, error) {
	...
	if err := cleaningPublisher.DeclareExchange(config.ExchangeConfig{
		Name:       cfg.CleaningExchangeConfig.Name,
		Kind:       cfg.CleaningExchangeConfig.Kind,
		Durable:    cfg.CleaningExchangeConfig.Durable,
		AutoDelete: cfg.CleaningExchangeConfig.AutoDelete,
		Internal:   cfg.CleaningExchangeConfig.Internal,
		NoWait:     cfg.CleaningExchangeConfig.NoWait,
	}); err != nil {
		_ = runCleanup(ctx, cleanup)
	...
	cleanup = append(cleanup, func(context.Context) error {
		closer, ok := clockEmu.(interface{ Close() error })
		if !ok {
			return nil
		}
		return closer.Close()
	...
	cleaningService, err := app.NewCleaningService(cleaningPublisher)
	if err != nil {
		_ = runCleanup(ctx, cleanup)
		return nil, fmt.Errorf("create cleaning service: %w", err)
	}
	guestService, err := app.NewGuestService(guestRepo, bookingRepo)
	if err != nil {
		_ = runCleanup(ctx, cleanup)
	...
	timeEventService := app.NewInMemoryTimeEventService()
	notificationService := app.NewNotificationService(timeEventService, redisClient)

	guestHandler := transporthttp.NewGuestHandler(guestService, clockEmu)
	wsServer := websocket.NewWebsocket(receptionService, cleaningService, clockEmu, notificationService, timeEventService)
	...
	registerRoutes(router, guestHandler, wsServer, probeHandler)
...
func registerRoutes(router *gin.Engine, guestHandler transporthttp.GuestHandler,
